...
watch([page, pageSize, () => route.query.q], async ([p, ps, q]) => {
  ...
  await fetchArticles({ page: p, pageSize: ps, category: 'entertainment', q: qStr })
})

onMounted(() => {
  const qStr = typeof route.query.q === 'string' ? route.query.q : undefined
  fetchArticles({ page: page.value, pageSize: pageSize.value, category: 'entertainment', q: qStr })
})
