...
watch([page, pageSize, () => route.query.q], async ([p, ps, q]) => {
  ...
  await fetchArticles({ page: p, pageSize: ps, category: 'business', q: qStr })
})

onMounted(() => {
  const qStr = typeof route.query.q === 'string' ? route.query.q : undefined
  fetchArticles({ page: page.value, pageSize: pageSize.value, category: 'business', q: qStr })
})
...
<template>
  <Grid col="1" gap="2" align-content="start">
    <Typography variant="heading-2" font-family="heading" font-weight="bold">Business</Typography>
